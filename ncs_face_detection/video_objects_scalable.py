...
import ncs_face_detection
from ncs_face_detection.video_processor import VideoProcessor
from ncs_face_detection.ssd_mobilenet_processor import SsdMobileNetProcessor
import cv2
import numpy
import time
import os
...
object_classifications_mask = [1, 1]

sys.prefix
NETWORK_GRAPH_FILENAME = "{}/graphs/ssd-face.graph".format(ncs_face_detection.__path__[0])
...
def overlay_on_image(display_image:numpy.ndarray, object_info_list:list, fps:float):
    ...
    for one_object in object_info_list:
        ...
        if one_object[0] == "face":
            label_background_color = (int(scale * 175), 0, 75)
...
def handle_args():
    ...
    global resize_output, resize_output_width, resize_output_height, min_score_percent, object_classifications_mask,\
           show_fps, show_device_count, device_count, rest_seconds, throttle_check_seconds, video_file
    ...
    for an_arg in argv:
        ...
        elif (str(an_arg).lower().startswith('video_file=')):
            try:
                arg, val = str(an_arg).split('=', 1)
                video_file = val
                print ('Video: {}'.format(val))
            except:
                print("Video file")
                return False;
...
#         return 1
    ...
    video_proc = VideoProcessor(video_file,
                                network_processor_list = obj_detect_list)
